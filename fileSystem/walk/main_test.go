...
func TestRun(t *testing.T) {
	testCases := []struct {
		...
		{name: "FilterExtensionMatch", root: "testdata", cfg: config{ext: ".log", size: 0, list: true},
			expected: "testdata/dir.log\n"},
		{name: "FilterExtenstionNoMatch", root: "testdata", cfg: config{ext: ".gz", size: 0, list: true},
			expected: ""},
		{name: "FilterExtenstionSizeMatch", root: "testdata", cfg: config{ext: ".log", size: 10, list: true},
			expected: "testdata/dir.log\n"},
		{name: "FilterExtenstionSizeNoMatch", root: "testdata", cfg: config{ext: "", size: 20, list: true},
			expected: ""},
	}
...
func TestRunDelExtension(t *testing.T) {
	testCases := []struct {
		...
		{name: "DeleteExtensionNoMatch", cfg: config{ext: ".log", del: true},
			extNoDelete: ".gz", nDelete: 0, nNoDelete: 10, expected: ""},
		{name: "DeleteExtensionMatch", cfg: config{ext: ".log", del: true},
			extNoDelete: "", nDelete: 10, nNoDelete: 0, expected: ""},
		{name: "DeleteExtensionMixed", cfg: config{ext: ".log", del: true},
			extNoDelete: ".gz", nDelete: 5, nNoDelete: 5, expected: ""},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(filesLeft) != tc.nNoDelete {
				t.Errorf("Expected %q files left, got %q instead\n", tc.nNoDelete, len(filesLeft))
			}
...
func TestRunArchive(t *testing.T) {
	testCases := []struct {
		...
		{name: "ArchiveExtensionNoMatch", cfg: config{ext: ".log"},
			extNoArchive: ".gz", nArchive: 0, nNoArchive: 10},
		{name: "ArchiveExtensionMatch", cfg: config{ext: ".log"},
			extNoArchive: "", nArchive: 10, nNoArchive: 0},
		{name: "ArchiveExtensionMixed", cfg: config{ext: ".log"},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(filesArchived) != tc.nArchive {
				t.Errorf("Expected %q files archived, got %q instead\n",
